...
fn test_rs4_close_all_board_commands() {
    let transport = crate::transport::mock::MockTransport {
        sent_frames: Vec::new(),
        queued_responses: Vec::new(),
    };
    let mut relay = crate::RelayBoardR4D8A08 {
        protocol: transport,
    };
    let _ = relay.close_all(1, 0);
    let response = relay.protocol.sent_frames.pop().expect("No value received");
