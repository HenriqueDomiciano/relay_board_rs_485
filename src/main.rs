...
fn main() {
    ...
    let mut relay_board = RelayBoardR4D8A08{protocol:transport};
    ...
    match args.relay_value {
        None => match args.command_type {
            CommandTypes::Action => match args.action_command {
                ...
                Some(action) => match action {
                    ActionCommandsEnum::CloseAll => {let _ = relay_board.close_all(args.slave_address,args.delay_time_ms);},
                    ActionCommandsEnum::OpenAll => {let _ = relay_board.open_all(args.slave_address,args.delay_time_ms);},
        ...
        Some(value) => match args.command_type {
            CommandTypes::Action => match args.action_command {
                None => {
                    let status = relay_board.read_status(args.slave_address,1, 8).unwrap().data[value as usize];
                    println!("{}", status);
                }
                Some(action) => match action {
                    ActionCommandsEnum::Close => {
                        let _ = relay_board.close_channel(args.slave_address,value, args.delay_time_ms);
                    }
                    ActionCommandsEnum::Open => {let _ = relay_board.open_channel(args.slave_address,value, args.delay_time_ms);},
                    ActionCommandsEnum::Latch => {
                        let _ = relay_board.latch_channel(args.slave_address,value, args.delay_time_ms);
                    }
                    ActionCommandsEnum::Toggle => {
                        let _ = relay_board.toogle_channel(args.slave_address,value, args.delay_time_ms);
                    }
                    ActionCommandsEnum::Delay => {let _ = relay_board.delay_time(args.slave_address,value, args.delay_time_ms);},
                    _ => panic!("Not supported type for specific relay command"),
                },
            },
            CommandTypes::Status => {
                let status = relay_board.read_status(args.slave_address,1, 8).unwrap().data[(value - 1) as usize];
                println!("{}", status);
            }
        },
    }
